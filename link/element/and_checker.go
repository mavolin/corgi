...
func (c *AndChecker) checkFor(f file.For, andAllowed bool) (_ bool, err error) {
	...
	// the element's body in there somewhere.
	...
	// check if we wrote to the body of the for, changing andAllowed inside the
	// loop
	if andAllowedBefore != andAllowed {
		// We did.
		// In this case the first item must not be an &, because if the first
		// isn't, then checkInElement wouldn't have allowed the others o be.
		if file.IsFirstAnd(f.Body) {
