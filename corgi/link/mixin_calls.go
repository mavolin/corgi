...
func (l *Linker) linkMixinCalls() error {
	...
	return l.linkMixinCallsScope(l.f.Scope, &scopes)
}

func (l *Linker) linkMixinCallsScope(s file.Scope, scopes *stack.Stack[file.Scope]) error {
	scopes.Push(s)

	for i, itm := range s {
		switch itm := itm.(type) {
		case file.Block:
			if err := l.linkMixinCallsScope(itm.Body, scopes); err != nil {
				return err
			}
		case file.Element:
			if err := l.linkMixinCallsScope(itm.Body, scopes); err != nil {
		...
		case file.If:
			if err := l.linkMixinCallsScope(itm.Then, scopes); err != nil {
				return err
			}

			for _, ei := range itm.ElseIfs {
				if err := l.linkMixinCallsScope(ei.Then, scopes); err != nil {
					return err
				}
			}

			if itm.Else != nil {
				if err := l.linkMixinCallsScope(itm.Else.Then, scopes); err != nil {
					return err
				}
			}
		case file.IfBlock:
		...
		case file.Switch:
			for _, c := range itm.Cases {
				if err := l.linkMixinCallsScope(c.Then, scopes); err != nil {
					return err
				}
			}

			if itm.Default != nil {
				if err := l.linkMixinCallsScope(itm.Default.Then, scopes); err != nil {
					return err
				}
			}
		case file.For:
